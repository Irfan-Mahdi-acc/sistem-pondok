...
import { bulkCreateLembagaGradesByCategory, deleteLembagaGradeByCategory } from "@/actions/lembaga-grade-actions-v2"
...
export function LembagaGradeInputTable({ 
  ...
  const handleSaveAll = () => {
    startTransition(async () => {
      ...
      const result = await bulkCreateLembagaGradesByCategory({
        categoryId,
        gradeType,
  ...
  const handleDeleteGrade = async (nilaiId: string) => {
    startTransition(async () => {
      const result = await deleteLembagaGradeByCategory(nilaiId)
